...
* [`src/cljs/roam_parse_fe/`](src/cljs/roam_parse_fe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/roam_parse_fe/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `roam-parse-fe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/roam_parse_fe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns roam-parse-fe.example
  (:require [roam-parse-fe.config :as config])
